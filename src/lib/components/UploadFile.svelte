<script lang="ts">
	...
	type PdfPreview = {
		...
		pdf: any | null;
		pageCount: number;
		currentPage: number;     // target (requested) page
		displayedPage: number;   // actually shown page (until target ready)
		...
		try {
			...
			if (browser) {
				for (const p of pdfPreviews) {
					if (p.pdf && p.pageCount > 0) {
						void ensurePageRendered(p, 1);
					}
				}
			}
		} finally {
	...
	async function ensurePageRendered(preview: PdfPreview, pageNumber: number) {
		...
		if (preview.pageCache[pageNumber]) {
			// Already rendered: if this page is the requested one and not displayed yet, display it
		...
		try {
			...
			// If user is still on (or waiting for) this page, show it
			...
			// Do not change displayedPage; keep previous
			touchPreview(preview);
		} finally {
	...
	function goToPage(preview: PdfPreview, target: number) {
		...
		if (preview.pageCache[target]) {
			// Already rendered → instant swap
			preview.displayedPage = target;
			touchPreview(preview);
		} else {
			// Keep old displayedPage until new is ready
...
<form
	class="flex flex-col gap-4 rounded-box bg-base-200 p-6 max-w-5xl mx-auto"
...
>
		...
		<div class="flex flex-col gap-8 mt-2 w-full">
			{#each pdfPreviews as preview, idx}
				<div class="flex flex-col items-center gap-3 w-full">
					...
					{#if preview.error}
						<div class="alert alert-error w-full max-w-[90vw] md:max-w-[1100px]">{preview.error}</div>
					{:else}
	...
	<button
		type="submit"
		class="btn btn-primary justify-center mt-4"
